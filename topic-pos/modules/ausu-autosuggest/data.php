...
if ($id && $data)
{
        ...
        {	
			...
			while($row = mysql_fetch_array($result)){
				...
				if($row2[$fieldName]!='')
					...
					$toReturn.=$row['goodFieldName'].' : <span style="color:#FFCC00; font-weight:bold">ثبت نشده</span> ';
				$i++;
				//echo ' ';
			}
            ...
            //$toReturn   = $row['goodName'].' <br /> نویسنده : '.$authorName.' | ناشر : '.$publisherName.' | مترجم : '.$translatorName.' | فروشنده : '.$sellerName;
	...
	elseif ($id=='oaName')
    {
        ...
        while ($row = mysql_fetch_array($result))
        {
			if($row['name']!='' || $row['family']!='') $name='<span style="color:#FFCC00">'.$row['name'].' '.$row['family'].'</span>'; else $name='<span style="color:#FFCC00; font-weight:bold">ثبت نشده</span>';
						
			if($row['company']!='') $company='<span style="color:#FFCC00">'.$row['company'].'</span>'; else $company='<span style="color:#FFCC00; font-weight:bold">ثبت نشده</span>';
			

			$toReturn   = 'نام : '.$name.' | شرکت : '.$company;
