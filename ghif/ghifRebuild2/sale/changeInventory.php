...
$pageHeader->createPageHeader('changeInventory','U');
...
$columns[5]='comments';
$values[0]=$goodId;
$values[1]=$_POST['employeId'];
$values[2]=$_POST['storeId'];
$values[3]=$_POST['stock'];
$values[4]=$pdate;
$values[5]=$_POST['comments'];
$tableName='inventory';
$idColumnName='inventoryId';
$table->insertIntoTable($columns,$values,$tableName,$idColumnName);
echo '<p align="center" class="addMsg">'.$changeInventory_successfully_changed_message.'</p>';
...
<p align="center" style="color:#D00"><?php echo $changeInventory_header; ?></p>
<form method="post" action="<?php $_SERVER['PHP_SELF'] ?>" id="changeInventoryForm">
...
<table border="0" cellspacing="3" cellpadding="0" align="center" dir="<?php echo $language_direction; ?>"  class="addTable" width="550px">
  <tr class="addTableHeader">
    <th colspan="2" align="center"><?php echo $changeInventory_title ?></th>
  </tr>
<?php
...
if(!$result2) echo 'Error No. 8';
    ...
    echo '
	<tr>
		<td width="50px">'.$changeInventory_name.'<br />'.$changeInventory_barcode.'</td>
	...
	<tr>
		<td>'.$changeInventory_stock.'</td>
		<td>
			<input type="txt" name="stock" id="inventoryCount" />
		</td>
	</tr>
    <tr>
   		<td>'.$changeInventory_store_name.'</td>
    	<td><select name="storeId" id="storeId" style="width:70px">';
		while ($row2=mysql_fetch_array($result2)){
			if($row1['storeId']==$row2['storeId'])
				$selected='selected="selected"';
			else $selected='';
			echo '<option value="'.$row2['storeId'].'" '.$selected.'>'.$row2['storeName'].'</option>';
		}
	echo '</select>
		</td>
	</tr>
	<tr>
		<td colspan="2">'.$changeInventory_comments.'</td>
	</tr>
	<tr>
		<td colspan="2">
	...
	<tr class="addTableTd">
    	<td colspan="2" align="center">
			...
			<input type="submit" value="'.$changeInventory_submit.'" class="addTableBut" />
		</td>
	</tr></table>
	</form>';
?>
