...
$pageHeader->createPageHeader('showCustomer','J');
include('../language/'.$_SESSION['language'].'/'.$_SESSION['language'].'.php');
$table=new table();
$fieldName1[0][0]='cName';
...
//$fieldName3[0][0]='../images/customers/';
$fieldName3[0][1]=$showCustomer_customer_photo;

if(isset($_GET['search']))$_SESSION['search']=$_GET['search'];
$search=$_SESSION['search'];
if($_SESSION['search']=='')
	$query="SELECT *, CONCAT(C.name,' ',C.family) AS cName FROM customers C ORDER BY customerId DESC";
else
	$query="SELECT *, CONCAT(C.name,' ',C.family) AS cName FROM customers C
	WHERE C.name LIKE '%$search%' OR C.family LIKE '%$search%' ORDER BY customerId DESC";
