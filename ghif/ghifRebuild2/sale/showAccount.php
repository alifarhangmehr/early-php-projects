...
$pageHeader->createPageHeader('showAccount','E');
...
if($_SESSION['search']=='')
	...
	LEFT JOIN employes E ON E.employeId = A.employeId ORDER BY accountId DESC";
else $query="SELECT A.price, A.payment, A.date, A.comments, CONCAT(O.name,' ', O.family)  AS oa, CONCAT(E.name,' ', E.family)  AS employe, accountId, F.factorNo FROM accounts A
	LEFT JOIN factor F ON F.factorId = A.factorId
	LEFT JOIN oa O ON O.oaId = A.oaId
	LEFT JOIN employes E ON E.employeId = A.employeId 
	WHERE O.name LIKE '%$search%' OR O.family LIKE '%$search%' OR F.factorNo LIKE '%$search%' OR A.date LIKE '%$search%' ORDER BY accountId DESC";
...
$table->searchTable($search,$query,$showAccount_account_count,$showAccount_search_input_placeholder,$pageInfo);
