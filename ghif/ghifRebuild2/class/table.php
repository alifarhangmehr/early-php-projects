	...
	function showTable($fieldName1,$fieldName2,$fieldName3,$pageInfo,$tableConfig){
		...
		if($pageName=='showFactor'){
			if($row['clear']==1){
				...
				if($row['canceled']==1){
					echo '<td align="center" class="tdwidth"><img src="../themes/'.$_SESSION['theme'].'/images/undo1.png" onclick="showUnCancelFactorDiv(\''.$row[$fieldId].'\')" style="border:hidden; cursor:pointer" title="'.$table_uncancel_factor_title.'" /></td>';
