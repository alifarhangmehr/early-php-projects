...
$pageHeader->createPageHeader('showOa','}');
...
if($_SESSION['search']=='')
	$query="SELECT *,CONCAT(oa.name,' ',oa.family) AS oName FROM oa";
else
	$query="SELECT * FROM oa
	WHERE name LIKE '%$search%' OR family LIKE '%$search%' OR company LIKE '%$search%' OR address LIKE '%$search%' OR tel LIKE '%$search%' OR mobile LIKE '%$search%'";
