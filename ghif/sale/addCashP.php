...
$pass=md5($_POST['pass']);
$reciverEmployeId=$_POST['reciverEmployeId'];
$cashierEmployeId=$_SESSION['adminId'];
$dateFrom=$_POST['dateFrom'];
$dateTo=$_POST['ِdateTo'];
$cashPrice=$_POST['cashPrice'];
$cardPrice=$_POST['cardPrice'];
$comments=$_POST['comments'];
$tCashPrice=$_POST['tCashPrice'];
$tCardPrice=$_POST['tCardPrice'];
$outgoCashPrice=$_POST['outgoCashPrice'];
$outgoCardPrice=$_POST['outgoCardPrice'];

$c2f=new Convertnumber2farsi();


$query="SELECT * FROM employes WHERE employeId='$reciverEmployeId' AND pass='$pass'";
$result=$table->ifValueExist($query);
if($result){	
	
	$updateField[0][0]='clear';
	$updateField[0][1]=1;
	$tableName='factor';
	$condition='date>='.$dateFrom.' AND date<='.$dateTo;
	...
	$columns[0]='reciverEmployeId';
	$columns[1]='cashierEmployeId';
	$columns[2]='dateFrom';
	$columns[3]='dateTo';
	$columns[4]='cashPrice';
	$columns[5]='cardPrice';
	...
	$values[0]=$reciverEmployeId;
	$values[1]=$cashierEmployeId;
	$values[2]=$dateFrom;
	$values[3]=$dateTo;
	$values[4]=$tCashPrice;
	$values[5]=$tCardPrice;
	...
	<table border="1" dir="'.$language_direction.'">
	  <tr>
		...
		<tr><td>'.$addCashP_cash_outgo.'</td><td>'.$c2f->Convertnumber2farsi($numToWord->number_format($outgoCashPrice)).''.$currency_name.'</td></tr>
		<tr><td>'.$addCashP_card_outgo.'</td><td>'.$c2f->Convertnumber2farsi($numToWord->number_format($outgoCardPrice)).''.$currency_name.'</td></tr>
		...
		<tr><td>'.$addCashP_cash_stock.'</td><td>'.$c2f->Convertnumber2farsi($numToWord->number_format($cashPrice)).''.$currency_name.'</td></tr>
		<tr><td>'.$addCashP_card_stock.'</td><td>'.$c2f->Convertnumber2farsi($numToWord->number_format($cardPrice)).''.$currency_name.'</td></tr>
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					series: [{
						...
						name: '<?php echo $addCashP_outgo; ?>',
						data: [-<?php if($outgoCashPrice=='') echo 0; else echo $outgoCashPrice; ?>,-<?php if($outgoCardPrice=='')echo 0; else echo $outgoCardPrice; ?>]
					}, ]
				});
...
<?php }
else echo '<br /><br /><br /><table border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td>'.$addCashP_wrong_password.'</td>
    <td align="center"><img src="../themes/default/images/notOkIcon.png" width="40" height="40" /></td>
  </tr>
  <tr>
    <td colspan="2" align="center"><a href="'.$_SERVER['HTTP_REFERER'].'">'.$addCashP_back.'</a></td>
  </tr>
</table>';
 ?>
