...
$con=new connect();
$con-> dbConnect();
$query = "SELECT * from  StructureghifUsers";
$result=$con->queryRun($query);
$inPageTrId=0;
while($row = mysql_fetch_array($result))
  {
	if($inPageTrId%2==0) $bgColor='bgcolor="#666666"';
	else $bgColor='';
	$inPageTrId++;
	
	if($row['image']=='') $image='<img src="../themes/default/images/admin/noStundetImage.png" width="60" height="60" />';
	else $image='<img src="../'.$row['image'].'thumb'.$row['ghifUserId'].'" width="45" height="60" />';
	$inPageId++;
	echo '<tr '.$bgColor.'>' ;
	echo '<td align=center id=imageTd'.$row['ghifUserId'].'>'.$image.'</td>';
	echo '<td align=center id=nameTd'.$row['ghifUserId'].'>'.$row['pName'].'</td>';
	echo '<td align=center id=familyTd'.$row['ghifUserId'].'>'.$row['pFamily'].'</td>';
	echo '<td align=center id=emailTd'.$row['ghifUserId'].'>'.$row['email'].'</td>';
