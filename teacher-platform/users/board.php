...
<title>بزرگترین مرجع ورد آف وارکرفت فارسی</title>
</head>
<body>
...
<?php
	...
	$con=new connect();
	$con-> dbConnect();
	$query = "SELECT * from usersboard ORDER BY id DESC";
...
<table width="660px" border="0" cellspacing="0" cellpadding="0" style="background:#1a1a1a; color:#FFF; font-size:12px">
  <tr>
...
1	<div id="colorToolTipDiv'.$row['id'].'" class="toolTipDiv">
  ...
  <tr>
    <td height="50%"><a style="color:#09F; font-size:12px" >گروه خبری : '.$row['newsGroup'].'</a></td>
		...
		$newsId = $row['id'];
		$query2 = "SELECT * from usersboardcomments WHERE newsId = '$newsId' AND cConfirm = 'yes' ORDER BY cId ASC";
