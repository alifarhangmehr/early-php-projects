...
function onParentNodeImageClick(node) {
	...
	var imageNode = node;
 	...
 	if (divNode.style.display == "none"){
		imageNode.src = "../modules/tree/minus.png";
		divNode.style.display = "";
	}
	else {
		imageNode.src = "../modules/tree/plus.png";
...
function setSelectedNode(imageNode){
    for (index = 0; index < this.nodeCount; index++) {
		document.getElementsByName("nodeImage" + index)[0].src = "../modules/tree/page.png";
    }
   	imageNode.src = "../modules/tree/pageSelected.png";
}
function expandAll(){
	for (index = 0; index < this.parentNodeCount; index++) {
		document.getElementById("parentNodeDiv" + index).style.display = "";
        document.getElementsByName("parentNodeImage" + index)[0].src = "../modules/tree/minus.png";     
	}
}
function collapseAll(){
	for (index = 0; index < this.parentNodeCount; index++) {
		document.getElementById("parentNodeDiv" + index).style.display = "none";
        document.getElementsByName("parentNodeImage" + index)[0].src = "../modules/tree/plus.png";     
	}
}
...
function startParentNode(text){
	...
	document.write('  <tr>');
	document.write('    <td><img src="../modules/tree/plus.png" name="parentNodeImage' + parentNodeCount + '" onclick="onParentNodeImageClick(this)" style="cursor:pointer;"/></td>');
...
function addNode(text, url, target){
	...
	document.write('<img src="../modules/tree/page.png" border="0" name="nodeImage' + this.nodeCount + '" onclick="onNodeImageClick(this);" />');
    document.write('</a>');
	document.write('	</td>');
...
function addExpandCollapseAll(){
	...
	document.write('  <tr>');
	document.write('    <td align="right" width="50%"><a onclick="expandAll();" class="expandCollapse" style="cursor:pointer;">نمایش همه زیر شاخه ها</a></td>');
