...
class connection{
	...
	function rLogin($user,$pass) {
		...
		//echo 'user : '.$user.' | pass : '.$pass;
		mysql_query("SET CHARACTER SET utf8");
		$result5 = mysql_query("select * from resellers where user='$user' and pass='$pass'"); 
        if (!$result5){
			$this->closeConnection();
			return false;}
	...
	function registerNew($user,$name,$family,$pass,$email,$tel,$mobile,$brief){
		...
		$groupname="public";
		$adminBrief="";
		//echo $active." | ".$user." | ".$name." | ".$family." | ".$pass." | ".$email." | ".$rDateEN." | ".$eDateEN." | ".$rDateFA." | ".$eDateFA." | ".$tel." | ".$mobile." | ".$bankName." | ".$bankSerial." | ".$acLevel." | ".$group." | ".$brief." | ".$adminBrief." | "; 
		
		
			//$query2="insert into users values 						                     ($active,$user,$name,$family,$pass,$email,$rDateEN,$eDateEN,$rDateFA,$eDateFA,$tel,$mobile,$bankName,$bankSerial,$acLevel,$group,$brief,$adminBrief)";
		
		
		 $query2="insert into users values 						                     ('$active','$user','$name','$family','$pass','$email','$rDateEN','$eDateEN','$rDateFA','$eDateFA','$tel','$mobile','$groupname','$brief','$adminBrief')";
		
		
		
		//$query2 = "INSERT INTO users (active,user,name,family,pass,email,rDateEN,eDateEN,rDateFA,eDateFA,tel,mobile,bankName,bankSerial,acLevel,group,brief) VALUES ('$active','$user','$name','$family','$pass','$email','$rDateEN','$eDateEN','$rDateFA','$eDateFA','$tel','$mobile','$bankName','$bankSerial','$acLevel','$group','$brief')";
		$con2=$this->link;
		mysql_query("SET CHARACTER SET utf8");
	...
	function queryRun($query){
		...
		mysql_query("SET CHARACTER SET utf8"); // baraye farsi kardane ersale etelaat be db 
	...
	function automaticRegister($user,$name,$family,$pass,$email,$tel,$mobile,$brief){
        if (!($this->dbConnect())) return false;
		$user = $this->check_input($user);
