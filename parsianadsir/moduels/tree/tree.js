...
function onParentNodeImageClick(node) {
	...
	var imageNode = node;
 	...
 	if (divNode.style.display == "none"){
		imageNode.src = "moduels/tree/minus.png";
		divNode.style.display = "";
	}
	else {
		imageNode.src = "moduels/tree/plus.png";
...
function setSelectedNode(imageNode){
    for (index = 0; index < this.nodeCount; index++) {
		document.getElementsByName("nodeImage" + index)[0].src = "moduels/tree/page.png";
    }
   	imageNode.src = "moduels/tree/pageSelected.png";
}
function expandAll(){
	for (index = 0; index < this.parentNodeCount; index++) {
		document.getElementById("parentNodeDiv" + index).style.display = "";
        document.getElementsByName("parentNodeImage" + index)[0].src = "moduels/tree/minus.png";     
	}
}
function collapseAll(){
	for (index = 0; index < this.parentNodeCount; index++) {
		document.getElementById("parentNodeDiv" + index).style.display = "none";
        document.getElementsByName("parentNodeImage" + index)[0].src = "moduels/tree/plus.png";     
	}
}
...
function startParentNode(text){
	...
	document.write('  <tr>');
	document.write('    <td><img src="moduels/tree/plus.png" name="parentNodeImage' + parentNodeCount + '" onclick="onParentNodeImageClick(this)" style="cursor:pointer;"/></td>');
...
function addNode(text, url, target){
	...
	document.write('<img src="moduels/tree/page.png" border="0" name="nodeImage' + this.nodeCount + '" onclick="onNodeImageClick(this);" />');
    document.write('</a>');
	document.write('	</td>');
